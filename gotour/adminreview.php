...
// Handle deletion of a review using prepared statements
if (isset($_GET['delete'])) {
    $delete_id = $_GET['delete'];
    
    // Prepare the delete query using PDO
    $delete_query = $connect->prepare("DELETE FROM review_table WHERE review_id = :delete_id");
...
<head>
    ...
    <link rel="stylesheet" href="asset/adminreview.css">
...
<h2>Users Review</h2>
                    <table class="table" id="myTable">
                        ...
                        <tbody>
                        <?php
                            // Fetch data securely using PDO and display it
                            $select_review = $connect->query("SELECT * FROM review_table", PDO::FETCH_ASSOC);

                            if ($select_review->rowCount() > 0) {
                                foreach ($select_review as $row) {
                                    echo "<tr>";
                                    echo "<td>" . htmlspecialchars($row['user_name']) . "</td>";
                                    echo "<td>" . htmlspecialchars($row['user_rating']) . "</td>";
                                    echo "<td>" . htmlspecialchars($row['user_review']) . "</td>";
                                    echo "<td>" . htmlspecialchars(date('l jS, F Y h:i:s A', $row["datetime"])) . "</td>";
                                    echo "<td>
                                            <a href='adminreview.php?delete=" . $row['review_id'] . "' 
